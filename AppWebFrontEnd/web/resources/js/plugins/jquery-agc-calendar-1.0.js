/*
 * jQuery calendar plug-in 1.0
 * http://www.agenciaclick.com.br
 *
 * Copyright (c) 2009 Danilo Martins <danilo.martins@agenciaclick.com.br>
 * Dual licensed under the MIT and GPL licenses.
 * http://docs.jquery.com/License
 *
 * Date: 2009-03-04 17:03:00 -0300 (Wed, 04 Mar 2009)
 */
...
jQuery.fn.calendario = function(options){
	...
	diaOriginal = parseInt(arrData[0].replace(/^0/, ''));
	mesOriginal = parseInt(arrData[1].replace(/^0/, ''))-1;
	...
	if(options.minDate!=''){
		...
		diaMinimo = parseInt(arrData[0].replace(/^0/, ''));
		mesMinimo = parseInt(arrData[1].replace(/^0/, ''))-1;
		anoMinimo = parseInt(arrData[2]);	
	}
	var diaMaximo = 9999; var mesMaximo = 9999;	var anoMaximo = 9999;
	if(options.maxDate!=''){
		arrData = options.maxDate.split('/');	
		diaMaximo = parseInt(arrData[0].replace(/^0/, ''));
		mesMaximo = parseInt(arrData[1].replace(/^0/, ''))-1;
	...
	this.each(function(){
		...
		function navegacaoCalendario(idCalendario){
			...
			//alert('MIN# Aano:' + ano + ' Ames:' + mes + ' Mano:' + anoMinimo + ' Mmes:' + mesMinimo);
			...
			//alert('MAX# Aano:' + ano + ' Ames:' + mes + ' Mano:' + anoMaximo + ' Mmes:' + mesMaximo);

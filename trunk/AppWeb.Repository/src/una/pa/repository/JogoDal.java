...
public class JogoDal {
    ...
    public static Jogo.enNivelUsuarioJogo nivelUsuarioJogo(int _idUsuario, int _idjogo) {
        String sql = "select * from ("
                + "select id_usuario, id_jogo, 'd' as tipo from jogo_desejado "
                + "union "
                + "select id_usuario, id_jogo, 'j' as tipo from jogo_usuario"
                + ") tabela where id_usuario = ? and id_jogo = ?";

        Object[] vetor = {_idUsuario, _idjogo};
        try {
            Connection c = Data.openConnection();
            ResultSet rs = Data.executeQuery(c, sql, vetor);

            Jogo.enNivelUsuarioJogo o = Jogo.enNivelUsuarioJogo.nenhum;
            if (rs.next()) {

                String tipo = rs.getString("tipo");
                o = (tipo.equals("j"))?Jogo.enNivelUsuarioJogo.jogo:(tipo.equals("d"))? Jogo.enNivelUsuarioJogo.desejo:Jogo.enNivelUsuarioJogo.nenhum;
            }
            rs.close();
            c.close();
            return o;
